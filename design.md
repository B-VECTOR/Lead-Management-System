...
| `<AllocationHealthBadge health title>` | Over/under-allocation verdict (Tech Req §4.7): over→red, under→amber, matched→emerald, nothing to measure→a muted `—`. Reads a `health` object from `allocationHealth(task)` in [lib/allocation.js](frontend/src/lib/allocation.js) — the arithmetic lives there, only the colour lives here. Excludes the optional named extras from both sides of the comparison |

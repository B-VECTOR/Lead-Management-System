...
- **Changed 2026-08-05 (post-v17.0):** those indicators are now **gated on when a slot's requirement is knowable and due** — §4.7 specified the arithmetic but not the timing, and applying it unconditionally made every freshly started project report itself several people short (all team-allocation steps are trigger-`pending` until their stage's start date; the auditor steps stay pending until the end of the project). A missing **Execution Red** is now reported specifically as such, being the one gap that blocks submit. Per the user. See the "When the comparison fires" table in §4.7.
...
**When the comparison fires (added 2026-08-05, R24 — per the user).** The rule above says what to compare, not when, and applying it unconditionally made a project report itself several people short on the day it started: every team-allocation step (3, 10, 17) is trigger-gated to its stage's start date and so sits **pending**, and the auditor steps (18, 25) sit pending for a project's whole life because audit comes at the end. Staffing a step in advance is expected, not early. So a slot is compared only once its requirement is both **knowable** and **due**:

| Slot | Compared when | Before that |
|---|---|---|
| Execution Red | always, while the step is in play | — it is fixed at 1 and **mandatory to submit**, so an empty one is reported specifically as **"Red required"**, not as a generic shortage. One exception: `carry_forward_red` runs when a step *opens*, so a still-`pending` step legitimately holds no Red row — if the lead has a Red to carry into it (`allocation.prefill.execution_red`) the slot reads "Carries over"; with nothing to inherit (a lead's first allocation step) it is a real gap |
| Execution Brown, White | the upstream manpower request has been **submitted** *and* the step is no longer `pending` | not compared — neither short nor excess |
| Auditor 1–2 | the step is no longer `pending` | not compared, and the pickers sit in the queue row's **Team & extras** panel rather than a main column |
| Auditor 3–4, Project Member 1–10 | never (optional by design, above) | — |

Two supporting mechanics:

- **`allocation.manpower_requested`** (payload flag). `man_power_required` reads **0** both when the request approved nobody and when nobody has been asked yet (`resources.slot_requirements` derives it from the *closed* source task). Those are opposite facts — the first makes staffing anyone an excess, the second makes it normal — so the server states which it is.
- **The `man_power_required` snapshot is corrected when the request lands.** It is written at row creation and copied forward by `reassign`, so a resource staffed before the request keeps a stale 0 for the life of the engagement, and the lead's Resources tab (which reads the column, not the live figure) would show a permanent red "1 of 0". `resources.sync_manpower_requirement`, called from `engine.complete_task`, backfills the currently-`allocated` rows of the affected steps. Released rows are left alone — they are the audit trail.

Nothing here changes what is *blocked*: the submit rule is still Execution Red (plus Auditors 1–2 on an auditor step), and over-allocation remains an indicator.

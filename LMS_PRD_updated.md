...
**An allocation step is measured only once it is due (added 2026-08-05, per the user).** Resources may be assigned well ahead of the step that needs them — that is the point of listing not-yet-due steps in the Resource Manager's queue — so a step nobody can act on yet does not report a shortage:

- **The Execution Red is always required.** It is mandatory and carries the engagement, so an unfilled one is flagged from the start, and flagged as exactly that ("Red required") rather than as an anonymous shortage. It is also the only thing that blocks submitting the step. The one thing that is *not* flagged is a step that isn't due yet whose Red simply continues from the previous stage — the system assigns that one itself when the step opens, and the slot says "Carries over" until it does.
- **Execution Brown and White are measured once the manpower request has been made** — i.e. once the upstream task that states the Brown/White numbers has been submitted — **and the step itself is due**. Assigning someone before then is neither under- nor over-allocation.
- **Auditors are not measured early.** Audit comes at the end of a project, so until the auditor step is actually due, Auditors 1–2 sit alongside the other named slots in the queue row's **Team & extras** panel — still assignable in advance, just not counted and not occupying a main column. When the step opens they move out into the Auditors column and are measured normally.

In every held-off case the screen still shows what the slot will need and who is in it; only the red/amber verdict waits. Hovering the step's Manpower indicator gives the per-slot breakdown, including why any slot is not being counted yet.
